<template>
    <label class="_radio">
        <input type="radio" :value="val" v-model="checked" @change="onChange">
        <cut><slot></slot></cut>
    </label>
</template>
...
._radio {display: flex; position: relative; padding: 0 0 0 24px;
    ...
    > ._cut {
        &:before {display: block; content: ''; position: absolute; top: 0.9ex; left: 0; width: $size; height: $size; margin-top: -($size / 2); background: url(~/static/images/common/radio.png) 0 0 no-repeat;}
    }
}
</style>
